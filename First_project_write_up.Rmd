...
From the graph above, we see that the following variables are relatively independent from others: gdppercapita_growth,forestcoverage,sdi,aged_15plus_unemployment_rate_percent,parliament.
...
```{r}
pr_pc=pr_happiness$x[,1:3]
pr_pc=cbind(pr_pc,df_variables_imputed%>%select(sdi,parliament))
pr_pc['hapiscore_whr']=df_variables$hapiscore_whr
model_pure_pcr=lm(hapiscore_whr~PC1+PC2+PC3,data=pr_pc)
model_pcr_additive=lm(hapiscore_whr~PC1+PC2+PC3+sdi+parliament,data=pr_pc)
...
set.seed(123)
lm_model_final_cv=train(hapiscore_whr~PC1+PC2+PC3+parliament,
      method="lm",
      data=train_pr,
...
## Implementation using tree-based modeling:
