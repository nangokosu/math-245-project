...
#Influence Measures Check & Remove Influence Measures
...
#Classification Tree
```{r}
df.cart <- train(hdiindex ~ hapiscore_whr+parliament+aged_15plus_unemployment_rate_percent+right+sanitation+edu+internetusers+sdi+forestcoverage+militaryexpenditure+mortality+wateraccess+broadband+agriculture+debt+gdppercapita_growth+foodinsecurity+corruption+incomeperperson+exports+vaccine, data=df_variables,method="rpart1SE", trControl =trainControl(method = "cv", number=5) )
df.cart
```
...
library(ROCR)
pred = prediction(pihat[,2], df_variables$hdiindex)
